...
assert(
  'revision 2 diff tracks modified',
  rev2.diff.modifiedBlocks.length >= 1,
...
assert(
  'resolve has resolvedAtRevision',
  resolveRes.comment?.resolvedAtRevision === metaAfter.currentRevision,
);

// Verify feedback excludes resolved
const fb1 = await store.getFeedback(art1.artifact.id);
assert(
  'feedback after resolve: art1 openComments=1',
  fb1.openComments.length === 1,
  `got ${fb1.openComments.length}`,
);
assert(
  'feedback excludes resolved comment',
  !fb1.openComments.some((c) => c.id === cmt1.comment.id),
);
...
assert(
  'feedback after reopen: openComments=2',
  fb2.openComments.length === 2,
...
assert(
  'create artifact 3 (nested) ok',
  art3.ok === true,
  JSON.stringify(art3.errors || art3.error),
);
if (!art3.ok) process.exit(1);
artifactIds.push(art3.artifact.id);

// Comment on deeply nested block (callout inside grid)
const cmtNested = await store.addComment(
  art3.artifact.id,
  `${t3}-nested-callout`,
  'Comment on nested callout',
);
assert('comment on nested block ok', cmtNested.ok === true);
assert(
  'nested comment has blockSnapshot',
  cmtNested.comment?.blockSnapshot?.id === `${t3}-nested-callout`,
);
assert(
  'nested comment blockSnapshot type=callout',
  cmtNested.comment?.blockSnapshot?.type === 'callout',
);

// Comment on tab content (code inside tab inside tabs)
const cmtTab = await store.addComment(
  art3.artifact.id,
  `${t3}-tab-code`,
  'Comment on tab code block',
);
...
assert(
  'nested feedback neighbors.prev is array',
  Array.isArray(nestedCmtFeedback?.neighbors?.prev),
);
assert(
  'nested feedback neighbors.next is array',
  Array.isArray(nestedCmtFeedback?.neighbors?.next),
);
...
assert(
  'grid parent comment blockSnapshot type=grid',
  cmtGrid.comment?.blockSnapshot?.type === 'grid',
);
...
assert(
  'orphan revision orphanedComments > 0',
  revOrphan.diff.orphanedComments.length >= 1,
...
assert(
  'feedback includes orphaned comments',
  orphanedCmts.length >= 1,
...
const revResolve = await store.addRevision(art4.artifact.id, revisedSource(t4), [
  cmtAuto.comment.id,
]);
...
assert(
  'comment resolved after revision',
  resolvedCmt?.status === 'resolved',
  `got ${resolvedCmt?.status}`,
);
...
const cmtEmptySelector = await store.addComment(
  art1.artifact.id,
  `${t1}-summary`,
  'Empty selector test',
  {
...
const cmtLongSelector = await store.addComment(
  art1.artifact.id,
  `${t1}-summary`,
  'Long selector test',
  {
...
const remaining = db
  .prepare(
    'SELECT COUNT(*) as c FROM artifacts WHERE id IN (' +
      artifactIds.map(() => '?').join(',') +
      ')',
  )
  .get(...artifactIds);
