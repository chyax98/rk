...
const CLI = 'node packages/cli/bin/renderkit.ts';
...
let srvParsed;
try {
  srvParsed = JSON.parse(srv.stdout);
} catch {
  srvParsed = null;
}
...
let feedbackParsed;
try {
  feedbackParsed = JSON.parse(feedback.stdout);
} catch {
  feedbackParsed = null;
}
...
const commentRes = await fetch(
  `${srvParsed.endpoint}/api/artifacts/${pushParsed.artifactId}/comments`,
  {
    method: 'POST',
...
assert(
  'selection comment stores selector',
  commentJson?.comment?.selector?.exact === 'RenderKit',
  JSON.stringify(commentJson?.comment?.selector || null),
);
const feedback2 = run(`${CLI} feedback ${FILE} --json`);
let feedback2Parsed;
try {
  feedback2Parsed = JSON.parse(feedback2.stdout);
} catch {
  feedback2Parsed = null;
}
assert(
  'feedback includes selector comment',
  (feedback2Parsed?.openComments || []).some(
    (c) => c.id === commentJson?.comment?.id && c.selector?.exact === 'RenderKit',
  ),
);
const resolveRes = await fetch(
  `${srvParsed.endpoint}/api/artifacts/${pushParsed.artifactId}/comments/${commentJson.comment.id}`,
  {
    method: 'PATCH',
    headers: { 'content-type': 'application/json' },
...
assert(
  'selection comment resolve ok=true',
  resolveJson?.ok === true && resolveJson?.comment?.status === 'resolved',
  resolveJson?.error || '',
);
const feedbackResolved = run(`${CLI} feedback ${FILE} --json`);
let feedbackResolvedParsed;
try {
  feedbackResolvedParsed = JSON.parse(feedbackResolved.stdout);
} catch {
  feedbackResolvedParsed = null;
}
assert(
  'resolved selector comment leaves feedback',
  !(feedbackResolvedParsed?.openComments || []).some((c) => c.id === commentJson?.comment?.id),
);
...
assert(
  'selection comment reopen ok=true',
  reopenJson?.ok === true && reopenJson?.comment?.status === 'open',
  reopenJson?.error || '',
);
...
assert(
  'd2 render returns svg',
  typeof d2?.svg === 'string' && d2.svg.startsWith('<svg'),
  `len=${d2?.svg?.length ?? 0}`,
);
const plantuml = await postDiagram('plantuml', '@startuml\nAlice -> Bob: hi\n@enduml');
assert('plantuml render ok=true', plantuml?.ok === true, plantuml?.error || '');
assert(
  'plantuml render returns svg',
  typeof plantuml?.svg === 'string' && plantuml.svg.startsWith('<svg'),
  `len=${plantuml?.svg?.length ?? 0}`,
);
