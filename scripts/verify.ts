...
function run(cmd) {
  try {
    return {
      stdout: execSync(cmd, { cwd: root, stdio: ['pipe', 'pipe', 'pipe'], encoding: 'utf8' }),
      code: 0,
    };
  } catch (e) {
...
for (const file of fixtures.good) {
  ...
  assert(
    `${file}: has blocks`,
    Array.isArray(parsed?.model?.blocks) && parsed.model.blocks.length > 0,
    `got ${parsed?.model?.blocks?.length ?? 'n/a'}`,
  );
...
const actualBadFixtures = readdirSync(fixtureDir)
  .filter((f) => f.endsWith('.rk.md'))
  .map((f) => `examples/fixtures/${f}`)
  .sort();
const mappedBadFixtures = Object.keys(fixtures.bad).sort();
assert(
  'fixture map has every fixture',
  JSON.stringify(actualBadFixtures) === JSON.stringify(mappedBadFixtures),
  `actual=${actualBadFixtures.join(',')} mapped=${mappedBadFixtures.join(',')}`,
);
...
if (existsSync(galleryPath)) {
  ...
  assert(
    'gallery has 7 surfaces',
    gallery.surfaces?.length === 7,
    `got ${gallery.surfaces?.length}`,
  );
...
for (const c of themeCases) {
  ...
  assert(
    `${c.file}: theme=${c.theme}`,
    parsed?.model?.theme === c.theme,
    `got ${parsed?.model?.theme}`,
  );
  assert(
    `${c.file}: covers multiple blocks`,
    (parsed?.model?.blocks?.length ?? 0) >= 5,
    `got ${parsed?.model?.blocks?.length ?? 'n/a'}`,
  );
}
...
let diagramParsed;
try {
  diagramParsed = JSON.parse(diagramCase.stdout);
} catch {
  diagramParsed = null;
}
const engines = new Set(
  (diagramParsed?.model?.blocks || [])
    .filter((b) => b.type === 'diagram')
    .map((b) => b.props?.engine),
);
...
let gridParsed;
try {
  gridParsed = JSON.parse(gridCase.stdout);
} catch {
  gridParsed = null;
}
...
assert(
  'grid blocks have children',
  grids.every((g) => (g.props?.children?.length ?? 0) >= 2),
);
...
let productParsed;
try {
  productParsed = JSON.parse(productCase.stdout);
} catch {
  productParsed = null;
}
const productTypes = new Set(
  (productParsed?.model?.blocks || []).flatMap((b) => [
    b.type,
    ...(b.props?.children || []).map((c) => c.type),
  ]),
);
assert(
  'product system case validates',
  productCase.code === 0 && productParsed?.ok === true,
  `exit=${productCase.code}`,
);
...
assert(
  'product system diagram shorthand infers mermaid',
  productDiagram?.props?.engine === 'mermaid',
);
assert(
  'product system diagram shorthand has body',
  (productDiagram?.props?.code || '').includes('flowchart LR'),
);
...
assert(
  ...
  productChart?.props?.engine === 'echarts-line',
);
assert(
  'product system chart shorthand has csv-like data',
  (productChart?.props?.code || '').includes('window,p50,p95'),
);
...
let richParsed;
try {
  richParsed = JSON.parse(richCase.stdout);
} catch {
  richParsed = null;
}
const richBlocks = richParsed?.model?.blocks || [];
assert(
  'rich media tabs case validates',
  richCase.code === 0 && richParsed?.ok === true,
  `exit=${richCase.code}`,
);
assert(
  'rich media tabs case has image block',
  richBlocks.some((b) => b.type === 'image'),
);
const tabsBlock = richBlocks.find((b) => b.type === 'tabs');
assert('rich media tabs case has tabs block', !!tabsBlock);
assert(
  'tabs block has two tabs',
  (tabsBlock?.props?.tabs?.length || 0) === 2,
  `got ${tabsBlock?.props?.tabs?.length || 0}`,
);
assert(
  'tabs contain nested blocks',
  (tabsBlock?.props?.tabs || []).every((t) => (t.blocks?.length || 0) >= 2),
);
...
let editorialParsed;
try {
  editorialParsed = JSON.parse(editorialCase.stdout);
} catch {
  editorialParsed = null;
}
const editorialBlocks = editorialParsed?.model?.blocks || [];
const editorialTypes = new Set(
  editorialBlocks.flatMap((b) => [b.type, ...(b.props?.children || []).map((c) => c.type)]),
);
assert(
  'editorial components case validates',
  editorialCase.code === 0 && editorialParsed?.ok === true,
  `exit=${editorialCase.code}`,
);
...
assert(
  'checklist parses checked and unchecked items',
  checklist?.props?.items?.some((i) => i.checked) &&
    checklist?.props?.items?.some((i) => !i.checked),
);
...
let autoIdParsed;
try {
  autoIdParsed = JSON.parse(autoIdCase.stdout);
} catch {
  autoIdParsed = null;
}
assert(
  'auto-id case validates',
  autoIdCase.code === 0 && autoIdParsed?.ok === true,
  `exit=${autoIdCase.code}`,
);
assert(
  'auto-id case generates deterministic directive id',
  (autoIdParsed?.model?.blocks || []).some((b) => b.id === 'auto-callout-no-id'),
);
...
let narrativeParsed;
try {
  narrativeParsed = JSON.parse(narrativeCase.stdout);
} catch {
  narrativeParsed = null;
}
const narrativeBlocks = narrativeParsed?.model?.blocks || [];
const narrativeTypes = new Set(
  narrativeBlocks.flatMap((b) => [b.type, ...(b.props?.children || []).map((c) => c.type)]),
);
assert(
  'narrative blocks case validates',
  narrativeCase.code === 0 && narrativeParsed?.ok === true,
  `exit=${narrativeCase.code}`,
);
...
assert(
  'comparison has columns from table headers',
  (compBlock?.props?.columns?.length || 0) >= 2,
  `got ${compBlock?.props?.columns?.length}`,
);
assert(
  'comparison has rows from table body',
  (compBlock?.props?.rows?.length || 0) >= 1,
  `got ${compBlock?.props?.rows?.length}`,
);
const tlBlock = narrativeBlocks.find((b) => b.type === 'timeline');
assert(
  'timeline has items',
  (tlBlock?.props?.items?.length || 0) >= 3,
  `got ${tlBlock?.props?.items?.length}`,
);
const tlStatuses = new Set((tlBlock?.props?.items || []).map((i) => i.status));
assert(
  'timeline items have varied statuses',
  tlStatuses.size >= 2,
  `got ${[...tlStatuses].join(',')}`,
);
...
assert(
  ...
  layoutSource.includes('openGraph') && layoutSource.includes('renderkit-og.svg'),
);
assert(
  'root layout declares Twitter card metadata',
  layoutSource.includes('twitter') && layoutSource.includes('summary_large_image'),
);
assert(
