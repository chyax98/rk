 ...
 * If running: push a TEMPORARY copy of alpha-showcase, status, feedback; assert JSON shapes.
 *
 * Uses a temp file so comments never pollute the original artifact.
 */
...
const SOURCE_FILE = "examples/alpha-showcase.rk.md";

// ── Create temp copy so we never pollute the original artifact ──
const tmpDir = mkdtempSync(join(tmpdir(), 'rk-smoke-'));
const tmpFile = join(tmpDir, 'smoke-test.rk.md');
copyFileSync(join(root, SOURCE_FILE), tmpFile);
...
// ── Push (temp file → fresh artifact each run) ──
console.log("\n== Push ==");
const push = run(`${CLI} push ${tmpFile} --json`);
let pushParsed;
...
// ── Status ──
console.log("\n== Status ==");
const status = run(`${CLI} status ${tmpFile} --json`);
let statusParsed;
...
// ── Feedback ──
console.log("\n== Feedback ==");
const feedback = run(`${CLI} feedback ${tmpFile} --json`);
let feedbackParsed;
...
const feedback2 = run(`${CLI} feedback ${tmpFile} --json`);
let feedback2Parsed;
...
const feedbackResolved = run(`${CLI} feedback ${tmpFile} --json`);
...
// ── Reopen is kept as a valid test (verify reopen round-trip) ──
...
// ── Final cleanup: resolve the smoke comment so it doesn't stay open ──
await fetch(`${srvParsed.endpoint}/api/artifacts/${pushParsed.artifactId}/comments/${commentJson.comment.id}`, {
  method: "PATCH",
  headers: { "content-type": "application/json" },
  body: JSON.stringify({ status: "resolved" })
});
