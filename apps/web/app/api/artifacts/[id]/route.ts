...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return Response.json({ ok: true, artifact: artifact.meta, revision: artifact.revision.number, comments: { open: artifact.comments.filter((c: any) => c.status === COMMENT_OPEN).length, resolved: artifact.comments.filter((c: any) => c.status === COMMENT_RESOLVED).length, orphaned: artifact.comments.filter((c: any) => c.status === COMMENT_ORPHANED).length } });
}
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!deleted) return Response.json({ ok: false, error: 'RK_ARTIFACT_NOT_FOUND' }, { status: 404 });
