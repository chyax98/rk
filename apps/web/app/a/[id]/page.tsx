...
function blockText(block: any): string {
  ...
  if (block.type === 'decision-card')
    return [props.question, props.chosen].filter(Boolean).join(' — ');
...
function artifactDescription(model: any): string {
  const text =
    (model?.blocks || []).map(blockText).find(Boolean) ||
    'Rendered RenderKit artifact with reading-first review comments.';
