...
import { blockLabel, copyToClipboard, flattenBlocks } from './lib';
...
export default function ArtifactView({
  ...
  const {
    ...
    commentsFor,
    blockCommentStatus,
    submitComment: rawSubmit,
    setCommentStatus,
  } = useComments(artifactId, initialComments);
  const {
    reviewMode,
    setReviewMode,
    drawerOpen,
  ...
  const outlineCommentCounts = Object.fromEntries(
    outlineItems
      .filter((i: any) => commentsFor(i.id).length > 0)
      .map((i: any) => [i.id, commentsFor(i.id).length]),
  );
    ...
    >
      ...
      <div className="rk-floating-tools" aria-label="文档工具">
        <button
          ...
          title="切换评审模式"
          className={reviewMode ? 'is-active' : ''}
        >
          评审
        </button>
        <button onClick={() => setOutlineOpen((o) => !o)} title="目录">
          ☰
        </button>
        <button onClick={() => openDrawer('comments', selected)} title="评论">
          💬{reviewMode && comments.length ? ` ${comments.length}` : ''}
        </button>
        <button onClick={() => copyToClipboard(feedbackCmd)} title="复制反馈命令">
          ⎘
        </button>
      </div>
      ...
      {wideReviewSurface && reviewMode && (
        <aside className="rk-review-pane">
          <ReviewPanel
            ...
            setDrawerMode={handleDrawerMode}
      ...
      {drawerOpen && !(wideReviewSurface && reviewMode) && (
        <aside className="rk-review-drawer">
          <div className="rk-drawer-head">
            <span>
              {drawerMode === 'comments' ? '评论' : drawerMode === 'agent' ? 'Agent' : '当前块'}
            </span>
