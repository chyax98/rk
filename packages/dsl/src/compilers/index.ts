 ...
 */
import type { BlockAttrs, BlockCompiler, CompileContext } from '../types.ts';
...
export const BLOCK_COMPILERS: Record<string, BlockCompiler> = {
  ...
  comparison: (node, attrs, ctx) =>
    compileComparison(node, attrs, ctx.source, ctx.errors, ctx.file),
  'decision-card': (node, attrs, ctx) =>
    compileDecision(node, attrs, ctx.source, ctx.errors, ctx.file),
