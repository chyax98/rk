...
import { pos, excerpt, plainText, diag } from '../helpers.js';
import { resolveDirective } from '../alias.js';
import { compileBlock } from './index.js';
export function compileTabs(node, attrs, ctx) {
    const tabs = [];
    for (const child of node.children || []) {
        if (child.type !== 'containerDirective' && child.type !== 'leafDirective')
            continue;
        if (child.name !== 'tab') {
    ...
    if (!tabs.length)
        ctx.errors.push(diag('RK_TABS_CHILD_REQUIRED', 'tabs directive requires at least one tab child', ctx.file, pos(node)));
    return {
        id: attrs.id,
        type: 'tabs',
