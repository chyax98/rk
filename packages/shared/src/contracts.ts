...
// ─── Themes ───
export const THEME_NAMES = Object.freeze(['paper-light', 'editorial-kami', 'dark-pro', 'amber-terminal'] as const);
export type ThemeName = typeof THEME_NAMES[number];

// ─── Surfaces ───
export const SURFACE_NAMES = Object.freeze(['engineering-plan', 'decision-brief', 'review-report', 'runbook', 'data-report-lite', 'proposal', 'documentation'] as const);
...
// ─── Diagram engines ───
export const DIAGRAM_ENGINES = Object.freeze(['mermaid', 'svg', 'echarts', 'echarts-bar', 'echarts-line', 'echarts-pie', 'infographic', 'plantuml', 'd2'] as const);
...
// ─── Callout tones ───
export const CALLOUT_TONES = Object.freeze(['info', 'warning', 'danger', 'success', 'neutral'] as const);
export type CalloutTone = typeof CALLOUT_TONES[number];

// ─── Defaults ───
export const DEFAULT_THEME: ThemeName = 'paper-light';
export const WIDE_REVIEW_SURFACES = Object.freeze(['engineering-plan', 'review-report', 'data-report-lite'] as const);

// ─── Table profiles & renderers (renderer schema) ───
export const TABLE_PROFILES = Object.freeze(['matrix', 'status', 'key-value', 'cards', 'compact'] as const);
...
// ─── Runtime validators ───
export function isKnownBlockType(type: string): type is BlockType { return BLOCK_TYPE_SET.has(type); }
export function isKnownThemeName(theme: string): theme is ThemeName { return THEME_SET.has(theme); }
export function isKnownSurfaceName(surface: string): surface is SurfaceName { return SURFACE_SET.has(surface); }
export function isKnownCommentStatus(status: string): status is CommentStatus { return COMMENT_STATUS_SET.has(status); }
export function isKnownDiagramEngine(engine: string): engine is DiagramEngine { return DIAGRAM_ENGINE_SET.has(engine); }
...
export function validateBlock(block: unknown, path = '$', issues: ContractIssue[] = []): ContractIssue[] {
  if (!isObject(block)) {
  ...
  if (children !== undefined) {
    ...
    else (children as unknown[]).forEach((child, index) => validateBlock(child, `${path}.props.children[${index}]`, issues));
  }
