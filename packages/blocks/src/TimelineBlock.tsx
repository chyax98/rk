...
export default function TimelineBlock({ title, items = [] }: Props) {
  return (
    <section className="rk-timeline-block">
      {title && (
        <h3 className="rk-timeline-heading">
          <RichText text={title} />
        </h3>
      )}
        ...
        {items.map((item, i) => {
          ...
          return (
            <article key={i} className={`rk-step${done ? ' is-done' : active ? ' is-active' : ''}`}>
              <div className="rk-step-num" aria-label={`步骤 ${i + 1}`}>
                {done ? '✓' : i + 1}
              </div>
              <div className="rk-step-body">
                <h4 className="rk-step-title">
                  <RichText text={item.label} />
                </h4>
                {item.body && (
                  <p className="rk-step-desc">
